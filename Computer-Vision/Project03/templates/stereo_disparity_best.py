...
def stereo_disparity_best(Il, Ir, bbox, maxd):
    ...
    # Sharpen the left and right images
    kernel = np.array([[0, -1, 0], 
                        [-1, 5, -1], 
                        [0, -1, 0]])
    Il = convolve(Il, kernel)
    Ir = convolve(Ir, kernel)
    ...
    # Post-processing - apply median and percentile filters
    Id[bbox[1,0]: bbox[1,1]+1, bbox[0,0]:bbox[0,1]+1] = \
        median_filter(Id[bbox[1,0]: bbox[1,1]+1, bbox[0,0]:bbox[0,1]+1], size=9) 
    Id[bbox[1,0]: bbox[1,1]+1, bbox[0,0]:bbox[0,1]+1] = \
        percentile_filter(Id[bbox[1,0]: bbox[1,1]+1, bbox[0,0]:bbox[0,1]+1],40, size=9) 
