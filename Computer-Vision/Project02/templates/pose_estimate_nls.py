...
def pose_estimate_nls(K, Twc_guess, Ipts, Wpts):
    ...
    while True:
        ...
        for i in np.arange(tp):
            # Get homogeneous pose matrix.
            Twc = hpose_from_epose(params)

            # Project world point to image frame.
            Wpt_augmented = np.vstack((Wpts[:, [i]], np.ones((1, 1))))
            Ipt = inv(Twc) @ Wpt_augmented
            Ipt = Ipt[:3, :]
            Ipt = K @ Ipt
            Ipt = Ipt / Ipt[2]

            # Compute residual.
            dY[2*i:2*i+2,:] = Ipt[:-1] - Ipts[:,[i]]
            
            # Compute jacobian.
            J[2*i:2*i+2,:] = find_jacobian(K, Twc, Wpts[:,[i]])
