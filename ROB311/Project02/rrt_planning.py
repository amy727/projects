...
def planning(rrt_dubins, display_map=False):
   ...
   while i < rrt_dubins.max_iter:
      ...
      # Since we know the goal node, we sometimes will use that instead of a random state
      if i<50 or random.random() > 0.2:
      ...
      for node in rrt_dubins.node_list:
         ...
         if dist < minDist:
            minDist = dist
            minDistNode = node
      new_node = rrt_dubins.propogate(minDistNode, rrt_dubins.Node(x,y,yaw))
